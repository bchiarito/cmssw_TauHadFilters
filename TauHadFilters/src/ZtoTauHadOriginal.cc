// -*- C++ -*-
//
// Package:    PreSelectionFilter/ZtoTauHadOriginal
// Class:      ZtoTauHadOriginal
// 
/**\class ZtoTauHadOriginal ZtoTauHadOriginal.cc PreSelectionFilter/ZtoTauHadOriginal/plugins/ZtoTauHadOriginal.cc
...
// class declaration
//
class ZtoTauHadOriginal : public edm::stream::EDFilter<> {
   public:
      explicit ZtoTauHadOriginal(const edm::ParameterSet&);
      ~ZtoTauHadOriginal();
...
// constructors
//
ZtoTauHadOriginal::ZtoTauHadOriginal(const edm::ParameterSet& iConfig) :
  cfg_tree(iConfig.getUntrackedParameter<bool>("tree")),
  ...
  cfg_etacut(iConfig.getUntrackedParameter<double>("absEtaMax"))
{
  ...
  tree_ = fs->make<TTree>("tree","ZtoTauHadOriginalTree");
  // event wide
  ...
  tree_->Branch("passHadTauKinematics",&bpassHadTauKinematics,"passHadTauKinematics/I");
...
// destructor
//
ZtoTauHadOriginal::~ZtoTauHadOriginal()
{
}

// ------------ method called on each new Event  ------------
bool
ZtoTauHadOriginal::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  // get event content
  ...
  for (double type : cfg_filterByTruthDecayType) if (type == bdecayType) passCfgDecayType = true;
  ...
  if (hadronicTaus.size() == 0)
  {
    ...
    {
      hadTauPassKinematics = hadTauPassKinematics || (hadronicTaus[i]->pt() > cfg_ptcut && abs(hadronicTaus[i]->eta())<cfg_etacut);
    }
  }

  bpassHadTauKinematics = hadTauPassKinematics; 

  // early return filter decision if not making tree
  if (cfg_filterByTruthDecayType.size() != 0 && !cfg_tree) return (passCfgDecayType && hadTauPassKinematics);
  ...
  if (cfg_filterByTruthDecayType.size() != 0) return (passCfgDecayType && hadTauPassKinematics);
  return bpassAll && hadTauPassKinematics;
}

vector<string>
ZtoTauHadOriginal::getDecay(const reco::Candidate & genparticle, int flag)
{
  ...
  //if (genparticle.numberOfDaughters() == 0) { products.push_back(""); return products; }
...
ZtoTauHadOriginal::getDecay(const reco::Candidate & genparticle, int flag)
{
  ...
  if (flag == 1) { // parent is tau
    if (genparticle.pdgId() == 111) {
      products.push_back("npion");
      return products;
    }
    if (abs(genparticle.pdgId()) == 211) {
      products.push_back("cpion");
      return products;
    }
  }
  ...
  if (abs(genparticle.pdgId()) == 15) {
    ...
    if (tau_products.size() == 0) {
      if (genparticle.pdgId() == 15) {
        products.push_back("tau-unid");
        return products; }
      if (genparticle.pdgId() == -15) {
        products.push_back("tau+unid");
        return products; }
    ...
    if (tau_products.size() == 1 && tau_products[0] == "e-") {
      products.push_back("tau-e");
      return products; }
    if (tau_products.size() == 1 && tau_products[0] == "e+") {
      products.push_back("tau+e");
      return products; }
    if (tau_products.size() == 1 && tau_products[0] == "mu-") {
      products.push_back("tau-mu");
      return products; }
    if (tau_products.size() == 1 && tau_products[0] == "mu+") {
      products.push_back("tau+mu");
      return products; }

    if (tau_products.size() == 1 && tau_products[0] == "tau-e") {
      products.push_back("tau-e");
      return products; }
    if (tau_products.size() == 1 && tau_products[0] == "tau+e") {
      products.push_back("tau+e");
      return products; }
    if (tau_products.size() == 1 && tau_products[0] == "tau-mu") {
      products.push_back("tau-mu");
      return products; }
    if (tau_products.size() == 1 && tau_products[0] == "tau+mu") {
      products.push_back("tau+mu");
      return products; }
    if (tau_products.size() == 1 && tau_products[0] == "tau+had") {
      products.push_back("tau+had");
      return products; }
    if (tau_products.size() == 1 && tau_products[0] == "tau-had") {
      products.push_back("tau-had");
      return products; }
    if (tau_products.size() == 1 && tau_products[0] == "tau+unid") {
      products.push_back("tau+unid");
      return products; }
    if (tau_products.size() == 1 && tau_products[0] == "tau-unid") {
      products.push_back("tau-unid");
...
void
ZtoTauHadOriginal::beginStream(edm::StreamID)
{
}

// ------------ method called once each stream after processing all runs, lumis and events  ------------
void
ZtoTauHadOriginal::endStream() {
}

// ------------ method called when starting to processes a run  ------------
void
ZtoTauHadOriginal::beginRun(edm::Run const&, edm::EventSetup const&)
{ 
}
 
// ------------ method called when ending the processing of a run  ------------
void
ZtoTauHadOriginal::endRun(edm::Run const&, edm::EventSetup const&)
{
}
 
// ------------ method called when starting to processes a luminosity block  ------------
void
ZtoTauHadOriginal::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a luminosity block  ------------
void
ZtoTauHadOriginal::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
ZtoTauHadOriginal::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.addUntracked<bool>("tree", false);
  ...
  descriptions.add("ZtoTauHadOriginalFilter", desc);
}

//define this as a plug-in
DEFINE_FWK_MODULE(ZtoTauHadOriginal);
